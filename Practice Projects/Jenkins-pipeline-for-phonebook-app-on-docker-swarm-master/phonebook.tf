...
# burada credential vermiyoz cunku bu file jenkins server icinde caliscak biz jenkins server i olustururken ona gerekli role leri aticaz. 
provider "aws" {
  region = "us-east-1"
  //  access_key = ""
  //  secret_key = ""
...
data "aws_caller_identity" "current" {} # Account ID gereken yerler var onu alabilmek icin bunu kullancaz
...
data "template_file" "leader-master" {
  template = <<EOF
    ...
    unzip awscliv2.zip
    ./aws/install
    # credential helper sayseinde makineler ecr a baglanabilcek. Bu sayede manager ve workerlar ECR dan image i pull edebilcek.
...
data "template_file" "manager" {
  template = <<EOF
    ...
    # ec2instanceconnectcli kuruyoz ki key pem olmadan bu makine gitsin swarm a katilabilmek icin mssh komutu ile token u alabilsin leaderden
...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "abdullah-Docker-Swarm-Manager-${count.index + 1}"
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "abdullah-Docker-Swarm-Worker-${count.index + 1}"
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "abdullah-docker-swarm-sec-gr"
  tags = {
    Name = "swarm-sec-gr"
  }
  dynamic "ingress" {
